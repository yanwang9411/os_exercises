...
>    初始化GDT，全局描述符表，保存所有segment的信息（其中Segment的初始地址保存在CPU的GDTR寄存器）完成段机制
>    采用buddy system 和 碎片整理 对物理内存空间实行管理
>    分配页作为页目录的table，建立对应的页表，建立0xc000000 - 0x00000000的映射，建立对等映射更新GDT，设置cr3 和 cr0寄存器的31bit来使能页机制，最后取消对等映射。
---
