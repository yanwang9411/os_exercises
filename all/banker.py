...
class Bankers(object):
    ...
    def ExecuteProcess(self,id):
        ...
        for rss in range(len(self.avaliable)):
            if (self.need[id][rss] > self.avaliable[rss] ):
                return False
        for rss in range(len(self.avaliable)):
            self.avaliable[rss] -= self.need[id][rss]
            self.allocated[id][rss] += self.need[id][rss]
            self.need[id][rss] = 0
        return True
    ...
    def TempSafeCheckAfterRelease(self):
        ...
        for id in range(len(self.max)):
            if ( self.finished[id] == False ):
                flag = False
